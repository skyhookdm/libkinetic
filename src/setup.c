/**
 * Copyright 2020-2022 Seagate Technology LLC.
 *
...
create_setup_message(kmsghdr_t *msg_hdr, kcmdhdr_t *cmd_hdr,
		     int64_t clustervers, void *fw, size_t fwlen);

kstatus_t extract_setup(struct kresult_message *resp_msg);


kstatus_t
s_setup_aio_generic(int ktd,

		    /* Args for Cluster Version Setup OP */
		    int64_t clustervers,

		    /* Args for Firmware Setup OP */
		    void *fw, size_t fwlen,
		    ...
		    void *cctx, kio_t **ckio)
{
	...
	if (!ckio) {
		debug_printf("setup: kio ptr required");
		return(K_EINVAL);
	}

	/*
	 * if fw is non NULL then this is a firmware upgrade req,
	 * otherwise it is a cluster version update req,
	 * nothing to validate wrt to the args
	 */
	...
	*ckio = NULL;
	...
	if (rc < 0) {
		debug_printf("setup: ktli config");
		return(K_EBADSESS);
	}
	...
	if (!kio) {
		kst->kst_setups.kop_err++;
		debug_printf("setup: kio alloc");
		return(K_ENOMEM);
	}
	...
	cmd_hdr.kch_type = KMT_SETUP;

	kmreq = create_setup_message(&msg_hdr, &cmd_hdr,
				     clustervers, fw, fwlen);

	if (kmreq.result_code == FAILURE) {
		debug_printf("setup: request message create");
		krc = K_EINTERNAL;
		goto sex_kio;
	}
	 ...
	 * is for the protobuf message. There is no value for cluster
	 * version but there is a value for firmware. These are mutually
	 * exclusive. FW must be in a single buffer so add an extra vector
	 * for it.
	 * See kio.h (previously in message.h) for more details.
	 */
	if (fw) {
		kio->kio_sendmsg.km_cnt = KM_CNT_NOVAL + 1;
	} else {
		/* Cluster Version Op */
	...
	if (!kio->kio_sendmsg.km_msg) {
		debug_printf("setup: sendmesg alloc");
		krc = K_ENOMEM;
		goto sex_kmreq;
	}
	...
	if (!kio->kio_sendmsg.km_msg[KIOV_PDU].kiov_base) {
		debug_printf("setup: sendmesg PDU alloc");
		krc = K_ENOMEM;
		goto sex_kmmsg;
	}

	/* Hang the fw if necessary */
	if (fw) {
		kio->kio_sendmsg.km_msg[KIOV_VAL].kiov_len  = fwlen;
		kio->kio_sendmsg.km_msg[KIOV_VAL].kiov_base = fw;
	}
	...
	if (pack_result == FAILURE) {
		debug_printf("setup: sendmesg msg pack");
		krc = K_EINTERNAL;
		goto sex_kmmsg_pdu;
	}
 ...
 sex_kio:
	...
	kst->kst_setups.kop_err++; /* Record the error in the stats */
...
/*
 * Complete a AIO setup call.
 * Any error other no available response, the KIO should be cleaned up
 * and terminated.
 */
kstatus_t
s_setup_aio_complete(int ktd, struct kio *kio, void **cctx)
{
	int rc, i;
	...
	if (!kio  || (kio && (kio->kio_magic !=  KIO_MAGIC))) {
		debug_printf("setup: kio invalid");
		return(K_EINVAL);
	}
	...
	if (rc < 0) {
		if (errno == ENOENT) {
			...
			debug_printf("setup: kio not available");
			return(K_EAGAIN);
		} else {
			 ...
			 */
			kst->kst_setups.kop_err++;
			debug_printf("setup: kio receive failed");
			return(K_EINTERNAL);
		}
	}
	...
	if (kio->kio_state == KIO_TIMEDOUT) {
		debug_printf("setup: kio timed out");
		kst->kst_setups.kop_err++;
		krc = K_ETIMEDOUT;
		goto nex;
	} else 	if (kio->kio_state == KIO_FAILED) {
		debug_printf("setup: kio failed");
		kst->kst_setups.kop_err++;
		krc = K_ENOMSG;
		goto nex;
	}
	...
	if (kiov->kiov_len != KP_PLENGTH) {
		debug_printf("setup: PDU bad length");
		krc = K_EINTERNAL;
		goto nex;
	}
	...
	if (kmresp.result_code == FAILURE) {
		debug_printf("setup: msg unpack");
		krc = K_EINTERNAL;
		goto nex;
	}

	krc = extract_setup(&kmresp);
 ...
 nex:
	...
	/*
	 * sendmsg always exists here and can have 1 and only 1 KIOV_VAL
	 * vectors(fw) but this value vector is the callers and can't be freed
	 * Also cheat here and use vl to keep separate count of FW length
	 */
	for (sl=0, i=0; i < kio->kio_sendmsg.km_cnt; i++) {
		...
		if (i < KM_CNT_NOVAL) {
			...
			/* should happen only once for i == KIOV_VAL */
			vl = kio->kio_sendmsg.km_msg[i].kiov_len;
		}
	}
	KI_FREE(kio->kio_sendmsg.km_msg);
	...
	if (krc == K_OK) {
		...
		/* Key Len is 0 for for SETUP but for value len see above */
		kl=0;

		kst->kst_setups.kop_ok++;
#if 1
		if (kst->kst_setups.kop_ok == 1) {
			kst->kst_setups.kop_ssize= sl;
			kst->kst_setups.kop_smsq = 0.0;

			kst->kst_setups.kop_rsize = rl;
			kst->kst_setups.kop_klen = kl;
			kst->kst_setups.kop_vlen = vl;

		} else {
			nmn = kst->kst_setups.kop_ssize +
				(sl - kst->kst_setups.kop_ssize)/kst->kst_setups.kop_ok;

			nmsq = kst->kst_setups.kop_smsq +
				(sl - kst->kst_setups.kop_ssize) * (sl - nmn);
			kst->kst_setups.kop_ssize  = nmn;
			kst->kst_setups.kop_smsq = nmsq;

			kst->kst_setups.kop_rsize = kst->kst_setups.kop_rsize +
				(rl - kst->kst_setups.kop_rsize)/kst->kst_setups.kop_ok;
			kst->kst_setups.kop_klen = kst->kst_setups.kop_klen +
				(kl - kst->kst_setups.kop_klen)/kst->kst_setups.kop_ok;
			kst->kst_setups.kop_vlen = kst->kst_setups.kop_vlen +
				(vl - kst->kst_setups.kop_vlen)/kst->kst_setups.kop_ok;
		}
#endif
		...
		if (KIOP_ISSET((&kst->kst_setups), KOPF_TSTAT)) {
			...
			s_stats_addts(&kst->kst_setups, kio);
...
kstatus_t
s_setup_generic(int ktd, int64_t clustervers, void *fw, size_t fwlen)

{
	kstatus_t ks;
	kio_t *kio;

	ks = s_setup_aio_generic(ktd, clustervers, fw, fwlen, NULL, &kio);
	if (ks != K_OK) {
		return(ks);
	}
	...
	do {
		 ...
		 */
		ks = s_setup_aio_complete(ktd, kio, NULL);
...
/**
 * kstatus_t
 * ki_setclustervers(int ktd, int64_t vers)
 *
 *  vers	New cluster version to be set on server
 *
 * Perform a round trip SETUP command.
 *
 *                       *******WARNING******
 * This current implementation is for expediancy and has race
 * conditions. Setting the cluster version has the effect of invalidating
 * any requests made with the old cluster version. So the real implementation
 * should create a barrier that prevents the creation of any new requests,
 * submit the setclustervers, wait for the response, if successful update
 * the kinetic session header (ksession_t) in the ktli config with the new
 * cluster version and then release the barrier. This should all be
 * controlled by KTLI
 *
 * This current implementation will fail any requests created after the
 * submission of the new cluster version and then subsequent completion at
 * the server, but before the kinetic session header in the ktli config can
 * be updated. Clients could get failed requests due to cluster version
 * violations. Could also cause problems if multiple set cluster versions
 * are called in a tight loop.
 *
 * ktli_config returns the ptr to the connection configuation structure.
 * So just updating the cluster version in this structure updates it for
 * all future requests. There are some atomic issues which are ignored in
 * this current implementation and would be handled by default in the
 * barrier implementation discussed above.
 */
kstatus_t
ki_setclustervers(int ktd, int64_t vers)
{
	...
	ks = s_setup_generic(ktd, vers, NULL, 0);

	/* If successful update the ktli config with the new cluster version */
	if (ks == K_OK) {
		ses->ks_ch.kch_clustvers = vers;
	}
	return(ks);
}
...
/**
 * kstatus_t
 * ki_getclustervers(int ktd, int64_t *vers)
 *
 *  *vers	Set to current cluster version
 *
 */
kstatus_t
ki_getclustervers(int ktd, int64_t *vers)
...
/**
 * kstatus_t
 * ki_firmware(int ktd, void *fw, size_t fwlen)
 *
 *  fw		a single buffer containing new firmware
...
ki_firmware(int ktd, void *fw, size_t fwlen)
{
	...
	return(s_setup_generic(ktd, 0, fw, fwlen ));
}
...
create_setup_message(kmsghdr_t *msg_hdr, kcmdhdr_t *cmd_hdr,
		     int64_t clustervers, void *fw, size_t fwlen)
{
	kproto_ksetup_t proto_cmd_body;
	ProtobufCBinaryData command_bytes;

	/* declare protobuf structs on stack */
	com__seagate__kinetic__proto__command__setup__init(&proto_cmd_body);

	/*
	 * This can either be a FIRMWARE or a Cluster Version op message.
	 * Setup the message according the params given.
	 */
	if (fw) {
		/*
		 * Set the Firmware OP as a fw ptr was sent in.
		 * Only need to set setupoptype, as the fw image will be
		 * added to the KIO as a Value
		 */
		set_primitive_optional(&proto_cmd_body,
				       setupoptype, KSOP_FIRMWARE);
	} else {
		/*
		 * Set the Cluster Version OP and hang the new version
		 * on the message
		 */
		set_primitive_optional(&proto_cmd_body,
				       setupoptype, KSOP_CLUSTERVERS);
		set_primitive_optional(&proto_cmd_body,
				       newclusterversion, clustervers);
	}
...
kstatus_t
extract_setup(struct kresult_message *resp_msg)
{

	kstatus_t krc = K_INVALID_SC;	/* assume failure status */
	kproto_msg_t *setup_resp_msg;
	kproto_cmd_t *resp_cmd;

	/* commandbytes should exist */
	setup_resp_msg = (kproto_msg_t *) resp_msg->result_message;
	if (!setup_resp_msg->has_commandbytes) {
		debug_printf("extract_setup: no resp cmd");
		return(K_EINTERNAL);
	}

	/*  unpack the command bytes */
	resp_cmd = unpack_kinetic_command(setup_resp_msg->commandbytes);
	if (!resp_cmd) {
		debug_printf("extract_setup: resp cmd unpack");
		return(K_EINTERNAL);
	}
	...
	if (krc != K_OK) {
		debug_printf("extract_setup: status");
	}
