 ...
 * Internal prototypes
 */
struct kresult_message create_noop_message(kmsghdr_t *, kcmdhdr_t *);
kstatus_t extract_noop(struct kresult_message *resp_msg);



kstatus_t
n_noop_aio_generic(int ktd, void *cctx, kio_t **ckio)
{
	...
	if (!ckio) {
		debug_printf("noop: kio ptr required");
		return(K_EINVAL);
	}
	...
	*ckio = NULL;
	...
	if (!kio) {
		kst->kst_noops.kop_err++;
		debug_printf("noop: kio alloc");
		return(K_ENOMEM);
	}
	 ...
	 * unfreeable ptr.  See below at nex_kmreq:
	 */
	...
	cmd_hdr.kch_type = KMT_NOOP;

	kmreq = create_noop_message(&msg_hdr, &cmd_hdr);
	if (kmreq.result_code == FAILURE) {
		debug_printf("noop: request message create");
		krc = K_EINTERNAL;
		goto nex_kio;
	}
	...
	if (!kio->kio_sendmsg.km_msg) {
		debug_printf("noop: sendmesg alloc");
		krc = K_ENOMEM;
		goto nex_kmreq;
	}
	...
	if (!kio->kio_sendmsg.km_msg[KIOV_PDU].kiov_base) {
		debug_printf("noop: sendmesg PDU alloc");
		krc = K_ENOMEM;
		goto nex_kmmsg;
	}
	...
	if (pack_result == FAILURE) {
		debug_printf("noop: sendmesg msg pack");
		krc = K_EINTERNAL;
		goto nex_kmmsg_pdu;
	}
 ...
 nex_kio:
	...
	kst->kst_noops.kop_err++; /* Record the error in the stats */
...
/*
 * Complete a AIO noop call.
 * Any error other no available response, the KIO should be cleaned up
 * and terminated.
 */
kstatus_t
n_noop_aio_complete(int ktd, struct kio *kio, void **cctx)
{
	int rc, i;
	...
	if (rc < 0) {
		if (errno == ENOENT) {
			...
			debug_printf("noop: kio not available");
			return(K_EAGAIN);
		} else {
			 ...
			 */
			kst->kst_noops.kop_err++;
			debug_printf("noop: kio receive failed");
			return(K_EINTERNAL);
		}
	}
	...
	if (kio->kio_state == KIO_TIMEDOUT) {
		debug_printf("noop: kio timed out");
		kst->kst_noops.kop_err++;
		krc = K_ETIMEDOUT;
		goto nex;
	} else 	if (kio->kio_state == KIO_FAILED) {
		debug_printf("noop: kio failed");
		kst->kst_noops.kop_err++;
		krc = K_ENOMSG;
		goto nex;
	}
	...
	if (kiov->kiov_len != KP_PLENGTH) {
		debug_printf("noop: PDU bad length");
		krc = K_EINTERNAL;
		goto nex;
	}
	...
	if (kmresp.result_code == FAILURE) {
		debug_printf("noop: msg unpack");
		krc = K_EINTERNAL;
		goto nex;
	}

	krc = extract_noop(&kmresp);
 ...
 nex:
	...
	if (krc == K_OK) {
		...
		/* Key Len and Value Len stats are 0 for NOOP*/
		kl=0;
		vl=0;

		kst->kst_noops.kop_ok++;
#if 1
		if (kst->kst_noops.kop_ok == 1) {
			kst->kst_noops.kop_ssize= sl;
			kst->kst_noops.kop_smsq = 0.0;

			kst->kst_noops.kop_rsize = rl;
			kst->kst_noops.kop_klen = kl;
			kst->kst_noops.kop_vlen = vl;

		} else {
			nmn = kst->kst_noops.kop_ssize +
				(sl - kst->kst_noops.kop_ssize)/kst->kst_noops.kop_ok;

			nmsq = kst->kst_noops.kop_smsq +
				(sl - kst->kst_noops.kop_ssize) * (sl - nmn);
			kst->kst_noops.kop_ssize  = nmn;
			kst->kst_noops.kop_smsq = nmsq;

			kst->kst_noops.kop_rsize = kst->kst_noops.kop_rsize +
				(rl - kst->kst_noops.kop_rsize)/kst->kst_noops.kop_ok;
			kst->kst_noops.kop_klen = kst->kst_noops.kop_klen +
				(kl - kst->kst_noops.kop_klen)/kst->kst_noops.kop_ok;
			kst->kst_noops.kop_vlen = kst->kst_noops.kop_vlen +
				(vl - kst->kst_noops.kop_vlen)/kst->kst_noops.kop_ok;
		}
#endif
		...
		if (KIOP_ISSET((&kst->kst_noops), KOPF_TSTAT)) {
			...
			s_stats_addts(&kst->kst_noops, kio);
...
kstatus_t
n_noop_generic(int ktd)
{
	kstatus_t ks;
	kio_t *kio;

	ks = n_noop_aio_generic(ktd, NULL, &kio);
	if (ks != K_OK) {
		return(ks);
	}
	...
	do {
		 ...
		 */
		ks = n_noop_aio_complete(ktd, kio, NULL);
...
/**
 * kstatus_t
 * ki_aio_noop(int ktd, void *cctx, kio_t **kio)
 *
 * Perform a round trip NOOP command.
 */
kstatus_t
ki_aio_noop(int ktd, void *cctx, kio_t **kio)
{
	return(n_noop_aio_generic(ktd, cctx, kio));
}
...
ki_noop(int ktd)
{
	return(n_noop_generic(ktd));
}


struct kresult_message
create_noop_message(kmsghdr_t *msg_hdr, kcmdhdr_t *cmd_hdr)
{
...
kstatus_t
extract_noop(struct kresult_message *resp_msg)
{

	kstatus_t krc = K_INVALID_SC;	/* assume failure status */
	kproto_msg_t *noop_resp_msg;
	kproto_cmd_t *resp_cmd;

	/* commandbytes should exist */
	noop_resp_msg = (kproto_msg_t *) resp_msg->result_message;
	if (!noop_resp_msg->has_commandbytes) {
		debug_printf("extract_noop: no resp cmd");
		return(K_EINTERNAL);
	}

	/*  unpack the command bytes */
	resp_cmd = unpack_kinetic_command(noop_resp_msg->commandbytes);
	if (!resp_cmd) {
		debug_printf("extract_noop: resp cmd unpack");
		return(K_EINTERNAL);
	}
	...
	if (krc != K_OK) {
		debug_printf("extract_noop: status");
	}
